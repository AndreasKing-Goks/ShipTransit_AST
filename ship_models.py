...
class EKFNoiseParameters(NamedTuple):
    process_noise_u: float          # Process noise for forward speed
    process_noise_v: float          # Process noise for sideways speed 
    process_noise_r: float          # Process noise for yaw rate
    process_noise_psi: float        # Process noise for yaw angle
    measurement_noise_u: float      # Measurement noise for forward speed
    measurement_noise_v: float      # Measurement noise for sideways speed
    measurement_noise_r: float      # Measurement noise for yaw rate

# # Define f, F_jacobian, h, and H_jacobian as methods within the class
# def F_jacobian(self, state, control_input):
#     """
#     Jacobian of the state transition function f
#     """
#     u, v, r, psi = state
#     thrust, rudder_angle = control_input
        
#     F = np.eye(4) # Placeholder: Identity matrix for simplicity
#     F[0, 0] -= damping_surge / mass * timestep
#     F[1, 1] -= damping_sway * timestep
#     F[2, 2] -= damping_yaw * timestep
#     F[3, 2] = timestep # dpsi/dr = dt
...
#     """
#     u,v,r,psi = state
#     return np.array([u, v, r]) # Measure speeds and yaw rate
...
#     def __init__(
#         self, state_dim, 
#         parameter: EKFNoiseParameters, 
#         timestep: float):
        
#         process_noise = np.array([parameter.process_noise_u, parameter.process_noise_v, parameter.process_noise_r, parameter.process_noise_psi])
#         measurement_noise = np.array([parameter.measurement_noise_u, parameter.measurement_noise_v, parameter.measurement_noise_r]) 
        
#         self.state = np.zeros(state_dim)            # State vector [u, v, r, psi]
#         self.P = np.eye(state_dim)                  # Covariance matrix
#         self.Q = np.diag(process_noise)             # Process noise covariance, TO TUNE
#         self.R = np.diag(measurement_noise)         # Measurement noise covariance, TO TUNE
#         self.dt = timestep                          # Time step
    
#     def predict(self, control_input, f, F_jacobian):
#         # Predict state using the state transition function
#         self.state = f(self.state, control_input)
        
#         # Compute Jacobian of f and update covariance
#         F = F_jacobian(self.state, control_input)
